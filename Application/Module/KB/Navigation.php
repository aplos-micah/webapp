<?php
/**
 * KB Module — Sidebar Navigation
 * Included by ControlPanel.php inside the sidebar nav list.
 */
?>

<li class="side-nav__group"><span>Knowledge Base</span></li>

<li>
    <a href="/kb/articles/list"
       class="side-nav__link<?= str_starts_with($currentSlug, 'kb/articles') ? ' is-active' : '' ?>">
        <i class="fa-solid fa-book side-nav__icon" aria-hidden="true"></i>
        <span>Articles</span>
    </a>
</li>

<li>
    <a href="/kb/guide"
       class="side-nav__link<?= $currentSlug === 'kb/guide' ? ' is-active' : '' ?>">
        <i class="fa-solid fa-book-open side-nav__icon" aria-hidden="true"></i>
        <span>User Guide</span>
    </a>
</li>
