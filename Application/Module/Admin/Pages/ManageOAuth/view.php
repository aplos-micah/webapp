...
(function () {
    function openEdit(id) {
        ...
        const edit = document.getElementById('token-edit-' + id);
        if (!row || !edit) return;
        row.hidden  = true;
        edit.hidden = false;
    ...
    document.querySelectorAll('tr[data-token-id]').forEach(function (row) {
        row.addEventListener('dblclick', function () { openEdit(row.dataset.tokenId); });
    });

    document.querySelectorAll('.token-edit-cancel').forEach(function (btn) {
        btn.addEventListener('click', function () {
            const edit = btn.closest('.li-edit-row');
            if (edit) closeEdit(edit.id.replace('token-edit-', ''));
        });
    });

    // Register client panel toggle
    const registerPanel  = document.getElementById('register-client-panel');
    const registerToggle = document.getElementById('register-client-toggle');
    const registerCancel = document.getElementById('register-client-cancel');
    if (registerToggle && registerPanel) {
        registerToggle.addEventListener('click', function () {
            registerPanel.hidden = !registerPanel.hidden;
            if (!registerPanel.hidden) registerPanel.querySelector('.input')?.focus();
        });
    }
