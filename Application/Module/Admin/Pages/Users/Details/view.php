...
<div class="card profile-card mb-lg">
    ...
    <dl class="field-list">
        ...
        <div class="field-list__row"><dt>Phone</dt><dd><?= $val($user['phone']) ?></dd></div>
        <div class="field-list__row"><dt>Job Title</dt><dd><?= $val($user['job_title']) ?></dd></div>
        <div class="field-list__row"><dt>Timezone</dt><dd><?= $val($user['timezone']) ?></dd></div>
    </dl>
    <?php endif; ?>
</div>
...
<div class="card profile-card mb-lg">
    ...
    <dl class="field-list">
        ...
        <div class="field-list__row">
            ...
            <dd><?= $user['email_verified_at'] ? $val($user['email_verified_at']) : '<span class="badge badge--warning">Unverified</span>' ?></dd>
        </div>
    </dl>
    <?php endif; ?>
</div>
        ...
        <div class="edit-section">
            ...
            <div class="form-row">
                <div class="form-group form-group--grow">
                    <label class="form-label">
                        ...
                        <span class="text-muted" style="font-size:0.78rem; margin-left:0.5rem;">
                            Set by <?= $e($grantedBy) ?> on <?= $e(substr($grantedAt, 0, 10)) ?>
                        </span>
                        <?php endif; ?>
                    </label>
                    <select name="module_access[<?= $e($modKey) ?>]" class="input">
                        ...
                        <option value="<?= $tier ?>"<?= $current === $tier ? ' selected' : '' ?>>
                            <?= $tier ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="form-group" style="align-self:flex-end; padding-bottom:0.5rem;">
                    <span class="badge <?= $tierBadge[$current] ?? 'badge--neutral' ?>"><?= $e($current) ?></span>
                </div>
                <?php endif; ?>
            </div>
            <?php endforeach; ?>
            ...
            <div class="form-actions--sm">
                <button type="submit" class="btn btn--primary">
                    <i class="fa-solid fa-floppy-disk" aria-hidden="true"></i> Save Module Access
                </button>
            </div>
        </div>
    </form>
