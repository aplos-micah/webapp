...
<script>
(function () {
    // ── Line item add/cancel toggle ──────────────────────────────────────────
    const toggle    = document.getElementById('line-item-toggle');
    const wrap      = document.getElementById('line-item-form-wrap');
    const cancelBtn = document.getElementById('line-item-cancel');

    if (toggle && wrap && cancelBtn) {
        toggle.addEventListener('click', function () {
            wrap.hidden = !wrap.hidden;
            if (!wrap.hidden) {
                wrap.querySelector('.product-lookup__input')?.focus();
            }
        });
    ...
    function openLineItemEdit(liId) {
        const dispRow = document.querySelector('tr[data-li-id="' + liId + '"]');
        const editRow = document.getElementById('li-edit-' + liId);
    ...
    function closeLineItemEdit(liId) {
        const dispRow = document.querySelector('tr[data-li-id="' + liId + '"]');
        const editRow = document.getElementById('li-edit-' + liId);
        if (!editRow) return;
        editRow.hidden = true;
    ...
    document.querySelectorAll('.li-edit-cancel').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            if (!editRow) return;
            closeLineItemEdit(editRow.id.replace('li-edit-', ''));
        });
    });

    // ── Mobile tab switching ─────────────────────────────────────────────────
    const tabBar    = document.getElementById('opp-tab-bar');
    const panelDetail    = document.getElementById('opp-panel-detail');
    const panelLineItems = document.getElementById('opp-panel-line-items');

    if (tabBar && panelDetail && panelLineItems) {
        tabBar.addEventListener('click', function (e) {
            const btn = e.target.closest('.opp-tab-bar__btn');
            if (!btn) return;
...
<?php endif; ?>
<?php include __DIR__ . '/../../Contacts/_partials/product-lookup.js.php'; ?>
